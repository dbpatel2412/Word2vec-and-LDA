...
files <- list.files(path="E://study metirial//admproject//email", full.names=T, recursive=FALSE)
for(file in files)
{   count_files_words(file) } 


dataset<- c("E://study metirial//admproject//email"
           )
...
ldaOut <-LDA(dtmpostproc,k, method="Gibbs", control=list(nstart=nstart, seed = seed, best=best, burnin = burnin, iter = iter, thin=thin))
#write out results
#docs to topics
ldaOut.topics <- as.matrix(topics(ldaOut))
write.csv(ldaOut.topics,file=paste("E://study metirial//admproject",k,"DocsToTopics.csv"))
#top 10 terms in each topic
ldaOut.terms <- as.matrix(terms(ldaOut,10))
write.csv(ldaOut.terms,file=paste("E://study metirial//admproject",k,"TopicsToTerms.csv"))
ldaOut.terms
